...
function createNativeComponentFromHTML(Component, component, parent, rawText, autoParent, parentIsLoopComponent = false, parentUniqIndex) {
	...
	let text = ``;
	let attr = {};
	let position = uuidv4().replace(/\-/igm,"");
	...
	let loopComponent = (attr.hasOwnProperty("$loopcomponent"))? {
		...
		componentId: `"${position}" + ${attr["$loopcomponent"] || 0}`
	} : {};


	for (let x in attr) {

		if (!(/(positioncomponent|state|on\:|parent|componentid|component\:|\$async|\$loopcomponent)/igm.test(x))) {
	...
	if (!(Component.tagName in HTMLElementTag)) {
		...
		template = `dabMain.createRawComponent(
			\`${Component.tagName}\`,
			{
				...
				positionComponent: ${loopComponent?.componentId || attr?.componentid?.replace(/\"/igm, "") || ((parentIsLoopComponent)? `"${position}" + ${parentUniqIndex || attr["$loopcomponent"]}` : '"' + position + '"')},
				state: ${attr["state"] || "{}"},
...
function createCustomComponent(Component, rawText) {
	...
	let attr = {$id:`"${uuidv4().replace(/\-/igm,"")}"`};
...
function transformComponent(rawComponent, res, positionComponent, indexComponent) {
	...
	rawComponent = rawComponent.replace(/\$beChild/igm, `parent="parentcomponent" componentid="positioncomponent" positioncomponent="positioncomponent"`)
